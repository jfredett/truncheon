...
async fn run_app<B: Backend>(terminal: &mut Terminal<B>, app: &mut UI) -> io::Result<bool> {
    ...
    tracing::debug!(target:"truncheon_representation::ui", "Logging to {}", dir.to_str().unwrap());
    tracing::debug!(target:"truncheon_representation::ui", "Logging initialized");

    // do an initial draw so we don't blank-screen, this maybe should be a splash page?
    tracing::info!(target:"truncheon_representation::ui", "initial draw");
    terminal.draw(|f| app.render(f) )?;

    loop {
        if app.check_flag("exit") { return Ok(true); }

        tracing::info!(target:"truncheon_representation::ui", "handling events");
        let event = event::read()?;
        app.handle_events(event);

        tracing::info!(target:"truncheon_representation::ui", "updating");
        app.update().await;

        tracing::info!(target:"truncheon_representation::ui", "drawing");
        terminal.draw(|f| app.render(f) )?;
